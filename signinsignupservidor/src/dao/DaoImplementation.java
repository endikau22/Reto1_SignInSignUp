...
public class DaoImplementation implements Signable {   
    ...
    /**
     * Comprobar que el email está registrado
     */
    private final String CONSULTAR_SI_ESTA_EMAIL = "Select * from usuario where BINARY email = ?";
    /**
     * Comprueba que el usuario está dado de alta. 
     ...
     */
    @Override
    public User signIn(User user) throws ExcepcionPasswdIncorrecta,ExcepcionUserNoExiste,Exception {
   ...
   /**
     ...
     * @throws excepciones.ExcepcionEmailYaExiste  
     */
    @Override
    public void signUp(User user) throws ExcepcionUserYaExiste,Exception,ExcepcionEmailYaExiste{
        ...
        try{
           //Guardar en el atributo de tipo Connection unna conexión de la pila.
            ...
            if(rs.next()){
                LOGGER.log(Level.INFO, "Lanzar error user ya existe al realizar un signUp");
                //Lanzar excepción user no existe
                throw new ExcepcionUserYaExiste();               
            }else{
               stmt = con.prepareStatement(CONSULTAR_SI_ESTA_EMAIL);
                //Añadir las variable login del user al statement
                ...
                //Si rs ( resultado de la query) no es vacío, hay en la Base de datos un usuario con el login.
                if(rs.next()){
                    LOGGER.log(Level.INFO, "Lanzar error email ya existe al realizar un signUp");
                    //Lanzar excepción user no existe
    ...
    /**
     * Actualiza la hora de salida de un usuario de la aplicación. 
     * @param user Un usuario
     * @throws SQLException 
     */
    @Override
